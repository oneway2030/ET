...
/**
 * Created by oneway on 2017/12/15.
 */

public class AES {
    public static AES instance;
    private String ivParameter = "070990c3e1721a09";// 密钥默认偏移，可以自定义
    String sKey = "070990c3e1721a09";// key必须为16位或者32位，可更改为自己的key
    private final String KEY_GENERATION_ALG = "PBEWITHSHAANDTWOFISH-CBC"; //加密方式方式  默认cbc
    private final String CIPHERMODEPADDING = "AES/CBC/PKCS5Padding";// AES/CBC/PKCS7Padding  //填充方式
    ...
    private AES() {
        try {
            ...
            Log.e("test", "invalid key spec for PBEWITHSHAANDTWOFISH-CBC");
        }
     ...
     * 加密
     *
     * @param plaintext 需要加密文本数组
     */
    public String encrypt(byte[] plaintext) {
        String s = "";
        try {
            byte[] ciphertext = encrypt(CIPHERMODEPADDING, skforAES, IV, plaintext);
            s = BASE64.encodeToString(ciphertext);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return s;
    ...
    public String decrypt(String str) {
        ...
        try {
            decrypted = new String(decrypt(CIPHERMODEPADDING, skforAES, IV, BASE64.decode(str)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return decrypted;
