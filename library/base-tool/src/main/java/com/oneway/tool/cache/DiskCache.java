...
import com.oneway.tool.utils.cipher.MD5;
import com.oneway.tool.utils.common.AppUtils;
...
public class DiskCache implements ICache {
    ...
    private long cacheTime = ToolConfig.CACHE_NEVER_EXPIRE;
    private Context mContext;

//    public DiskCache(Context context) {
//        this(context, getDiskCacheDir(context, ToolConfig.CACHE_DISK_DIR),
//                calculateDiskCacheSize(getDiskCacheDir(context, ToolConfig.CACHE_DISK_DIR)));
//    }

    public DiskCache() {
        mContext = ToolConfig.getContext();
        long diskMaxSize = calculateDiskCacheSize(getDiskCacheDir(ToolConfig.getContext(), ToolConfig.CACHE_DISK_DIR));
        File diskCacheDir = getDiskCacheDir(mContext, ToolConfig.CACHE_DISK_DIR);
        final String REGEX = "@createTime\\{(\\d+)\\}expireMills\\{((-)?\\d+)\\}@";
        compile = Pattern.compile(REGEX);
        try {
            cache = DiskLruCache.open(diskCacheDir, AppUtils.getAppVersionCode(), 1, diskMaxSize);
        } catch (IOException e) {
            e.printStackTrace();
            LogUtil.e(e);
        }
    ...
    public void put(String key, Object value) {
        put(key, value != null ? GsonUtil.getInstance().toJson(value) : null);
    }
            ...
            if (snapshot != null) {
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    if ((createTime + expireMills > Calendar.getInstance().getTimeInMillis())
                            || expireMills == ToolConfig.CACHE_NEVER_EXPIRE) {
